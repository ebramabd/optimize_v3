...
@section('content')
    <!-- Main Content -->
    <div class="p-7">
        <h1 class="home-h1 mb-8">{{ __('common.settings') }} / {{ __('common.add_new_administrators') }}</h1>

        <form class="card p-4 mb-4"
              action="{{route('companies.setting.company.save.administrator')}}"
            ...
            <div class="home-subscription d-flex align-items-center justify-content-between">
                ...
                <button class="service-save-btn btn btn-outline-secondary" style="font-size: 15px !important;">{{ __('common.save') }}</button>
            </div>

            <div class="p-8">
                <div class="link-box" id="click-add">
                    {{ __('common.add_new_administrators') }}
                </div>
            </div>

            <div class="d-none py-10 px-20 d-flex align-items-center" id="new-branch-input">
                <input type="text" class="input-service"
                       placeholder="{{ __('common.administrator_name') }}" width="90%" id="branch-input">
    ...
    <script>
        $(document).ready(function () {
            ...
            function addCloneBranch(branch_name) {
                ...
                clone.append(`<input type="hidden" name="administrator_name[]" value="${branch_name}">`);
