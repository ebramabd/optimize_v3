...
@section('style')
    <style>
        #phone-code-verification-modal .modal-header {
            margin-bottom: 0;
        }
            ...
            <form class="form" id="myForm"
                ...
                <div class="row g-8 p-2">
                    ...
                    <div class="col-md-6 position-relative d-flex align-items-center">
                        ...
                        <span class="confirm-btn confirm-phone-code-btn {{ app()->getLocale() === 'ar' ? 'me-5 ms-0' : '' }}">{{ __('common.confirm') }}</span>
                    </div>
                    <div class="col-md-6">
                        <input type="email" class="input-service client-info" name="email" value="{{old('email')}}" style="height: 45px" placeholder="{{ __('common.email') }}">
                    </div>
                </div>
                ...
                <div class="row g-3">
                    <div class="div-select-service col-md-6 mb-4">
                        <select name="services" required id="addService" class="input-service-select">
                            <option disabled selected>Select New Service</option>
                ...
                <div class="custom-select col-md-12">
                    <div class="select-box" onclick="toggleDropdown()">{{ __('admin.application_area') }} </div>
                    <div class="options-container">
                      ...
                      <label><input type="checkbox" name="application_area[]" value="Front Windshield"> {{ __('admin.front_windshield') }} </label>
                        ...
                        <label><input type="checkbox" name="application_area[]" value="Rear Door (R)"> {{ __('admin.rear_door_r') }} </label>  
                      <label><input type="checkbox" name="application_area[]" value="Rear Door (L)"> {{ __('admin.rear_door_l') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rocker Panel (R)"> {{ __('admin.rocker_panel_r') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rocker Panel (L)"> {{ __('admin.rocker_panel_l') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rear Fender (R)"> {{ __('admin.rear_fender_r') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rear Fender (L)"> {{ __('admin.rear_fender_l') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rear Bumper "> {{ __('admin.rear_bumper') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rear Trunk "> {{ __('admin.rear_trunk') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rear Spoiler "> {{ __('admin.rear_spoiler') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Front Spoiler "> {{ __('admin.front_spoiler') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Rear Diffuser "> {{ __('admin.rear_diffuser') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Pillars"> {{ __('admin.pillars') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Side Sills"> {{ __('admin.side_sills') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Side Mirrors"> {{ __('admin.side_mirrors') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Front Headlight"> {{ __('admin.front_headlight') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Door Handle Inserts"> {{ __('admin.door_handle_inserts') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Interior: Center Console"> {{ __('admin.Interior_Center_Console') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Interior: Door Panels"> {{ __('admin.Interior_Door_Panels') }} </label>
                      <label><input type="checkbox" name="application_area[]" value="Interior: Others"> {{ __('admin.Interior_Others') }} </label>
                ...
                <div class="col-md-12">
                    <select class="form-select" required name="center_id" id="branch-select">
                        @isset($companies)
                            <option value="">Select Branch</option>
                            @foreach($companies as $company)
                                <option value="{{$company->id}}">{{$company->branch_name}}</option>
                            @endforeach
                        @endisset
                    </select>
                </div>
                ...
                <div class="col-md-12">
                    <select class="form-select" required name="administrator" id="admin-select">
                        <option value="">Select Administrator</option>
                        @isset($administrator)
                            @foreach($administrator as $admin)
                                <option value="{{$admin->id}}">{{$admin->administrator_name}}</option>
                            @endforeach
                        @endisset
                    </select>
                </div>
    ...
    <script>
        ...
        $(function() {
            ...
            async function confirmPhoneCode() {
                let url = getUrl();
                let phoneNumber = $('#myForm input[name=phone]').val();
                if (phoneNumber.trim() === '') {
    ...
    <script>
        ...
        function addAreasToServiceItem(){
            ...
            setTimeout(() => {
                ...
                cloneArea.find(".area-name").text(selectedAreas);
        ...
        /*function validateForm(event) {
            const checkboxes = document.querySelectorAll('input[name="application_area[]"]');
            const anyChecked = Array.from(checkboxes).some(checkbox => checkbox.checked);

            if (!anyChecked) {
                alert("Please select at least one application area.");
                // Optional: Add red border to the select-box
                document.querySelector('.select-box').style.border = "2px solid red";
                event.preventDefault(); // This stops form submission
            } else {
                document.querySelector('.select-box').style.border = ""; // Remove red border if valid
            }
        }*/
        ...
        function checkValidatedData() {
            ...
            let brandSelected = $(".item-selected").last();
            let visibleNewService = $('.new-service:visible');
            let year_of_manufacture = $('input[name="year_of_manufacture"]');
            ...
            if(year_of_manufacture.val()<1900 || year_of_manufacture.val()==''){
                alert('Please Check Year of Manufacturer (It must be 1900 or higher)!');
                return false;
            }
            return true;
        }
        ...
        /*document.addEventListener("DOMContentLoaded", () => {
            const form = document.querySelector("#myForm");
            form.addEventListener("submit", function(event) {
                event.preventDefault(); // Optional: to stop actual form submission for validation

                checkValidatedData();
                form.submit(); // Uncomment this if you want to submit the form after validation
            });
        });
        ...
        /*$(document).ready(function () {
            ...
            // Store current selected service element
            let currentServiceBlock = null;

            $(document).on('change', '.input-service-select', function (e) {
                // Reset application areas
                $('input[name="application_area[]"]').prop('checked', false).prop('disabled', false);
                // get all $('input[name="application_area[]"]') checked
                // add then after checked to .new-service as p
            });
            ...
            function updateHiddenAppArea() {
                if (!currentServiceBlock) return;

                // Get all selected application areas
                const selectedAreas = [];
                $('input[name="application_area[]"]:checked').each(function () {
                    selectedAreas.push($(this).val());
                });

                // Convert to JSON and store in the hidden input
                currentServiceBlock.find('.hidden-app-area').val(JSON.stringify(selectedAreas));
            }
        });*/
